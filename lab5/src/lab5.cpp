...
int main(int argc, char **argv) {
  ...
  ros::Publisher pub = node_handle.advertise<geometry_msgs::Point>("LAB5", 1000);

  current_pose = move_group.getCurrentPose();
  pub.publish(move_group.getCurrentPose().pose.position);
  ROS_INFO("Published: x: %f, y: %f", move_group.getCurrentPose().pose.position.x, move_group.getCurrentPose().pose.position.y);
